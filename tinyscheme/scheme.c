...
static pointer opexe_0(scheme *sc, enum scheme_opcodes op);//lang
static pointer opexe_1(scheme *sc, enum scheme_opcodes op);//lang
static pointer opexe_2(scheme *sc, enum scheme_opcodes op);//constructor, accessor, modifier
static pointer opexe_3(scheme *sc, enum scheme_opcodes op);//predicate
static pointer opexe_4(scheme *sc, enum scheme_opcodes op);//port io
static pointer opexe_5(scheme *sc, enum scheme_opcodes op);//io?
...
static pointer mk_memblock(scheme *sc, int len, char fill) {
     ...
     strvalue(x) = p;
     strlength(x) = len;
     memset(p, fill, len);//<string.h> 初始化字符串
     return (x);
}
...
static pointer opexe_2(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_MKBLOCK: { /* make-block*/
          ...
          if(!is_number(car(sc->args))) {
               Error_1(sc,"make-block: expected a number:",car(sc->args));
          }
          len = ivalue(car(sc->args));
          if(len<=0){
               Error_1(sc,"make-block: length must be positive:",car(sc->args));
          }
     ...
     case OP_BLOCKREF: { /* block-ref */
          ...
          if(index<0 || index>=strlength(car(sc->args))) {
...
#include "opdefines.h"
  { (void*)(0), (void*)(0), (void*)(0), (void*)(0), (void*)(0) }//final entry
};
